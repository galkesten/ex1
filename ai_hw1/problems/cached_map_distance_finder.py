...
class CachedMapDistanceFinder:
    ...
    def get_map_cost_between(self, src_junction: Junction, tgt_junction: Junction) -> Optional[Cost]:
        ...
        new_problem = MapProblem(self.streets_map, src_junction.index, tgt_junction.index)
        new_cost = self.map_problem_solver.solve_problem(new_problem).solution_cost
        new_tupple = (src_junction.index, tgt_junction.index)
        self._insert_to_cache(new_tupple, new_cost)
        return new_cost
