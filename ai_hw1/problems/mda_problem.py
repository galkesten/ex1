...
class MDAProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        left_capacity = self.problem_input.ambulance.total_fridges_capacity - \
        ...
        for apartment in self.get_reported_apartments_waiting_to_visit(state_to_expand):
            if apartment.nr_roommates <= state_to_expand.nr_matoshim_on_ambulance \
                    and apartment.nr_roommates <= left_capacity:
                new_state = MDAState(apartment,
                                     frozenset.union(state_to_expand.tests_on_ambulance, frozenset([apartment])),
                ...
                yield OperatorResult(successor_state=new_state,
                                     ...
                                     operator_name="visit " + new_state.current_site.reporter_name)
        for lab in self.problem_input.laboratories:
            if state_to_expand.tests_on_ambulance != frozenset() or lab not in state_to_expand.visited_labs:
                is_not_visited = lab not in state_to_expand.visited_labs
                new_state = MDAState(lab, frozenset(), frozenset.union
                (state_to_expand.tests_on_ambulance, state_to_expand.tests_transferred_to_lab),
                                     state_to_expand.nr_matoshim_on_ambulance + (is_not_visited * lab.max_nr_matoshim),
                                     frozenset.union(state_to_expand.visited_labs, frozenset([lab])))
                yield OperatorResult(successor_state=new_state,
                                     operator_cost=self.get_operator_cost(state_to_expand, new_state),
                                     operator_name="go to lab " + new_state.current_site.name)
